...
# ==========================================
    ...
    # ==========================================
        ...
        if not df_per.empty and not df_gen.empty:
            ...
            if col_fnac:
                # --- LÓGICA CONFIRMADA DE GOOGLE SHEETS ---
                # Ya sabemos que tus columnas se llaman exactamente APELLIDOS y NOMBRES
                if "APELLIDOS" in df_per.columns and "NOMBRES" in df_per.columns:
                    df_per["Trabajador"] = df_per["NOMBRES"].astype(str).str.strip() + " " + df_per["APELLIDOS"].astype(str).str.strip()
                else:
                    df_per["Trabajador"] = "Nombre no especificado"
                
                # --- LÓGICA PARA LA FOTO Y CONTACTO ---
                col_foto = "FOTO" if "FOTO" in df_per.columns else None
                col_cel = next((c for c in df_gen.columns if any(x in str(c).lower() for x in ["celular", "telefono", "móvil"])), None)
                col_em = next((c for c in df_gen.columns if any(x in str(c).lower() for x in ["correo", "email"])), None)
                ...
                # Asegurarnos de que las columnas base existen (DNI o dni)
                col_dni_per = "DNI" if "DNI" in df_per.columns else "dni"
                col_dni_gen = "DNI" if "DNI" in df_gen.columns else "dni"
                
                df_cumple = df_per[[col_dni_per, "Trabajador"] + (["Foto_URL"] if col_foto else [])].copy()
                df_cumple.rename(columns={col_dni_per: "DNI"}, inplace=True)
                
                cols_gen_a_jalar = [col_dni_gen, col_fnac]
                ...
                df_gen_temp.rename(columns={col_dni_gen: "DNI"}, inplace=True)
                ...
                # =====================================================================
                # IMPORTANTE: Sube tus nuevos diseños a GitHub con estos nombres exactos
                # =====================================================================
                img_mes_url = "https://raw.githubusercontent.com/agalindoUR/SISTEMA_GTH/main/fondo_mes.jpg" 
                img_ind_url = "https://raw.githubusercontent.com/agalindoUR/SISTEMA_GTH/main/fondo_ind.jpg"
                ...
                if not df_cumple.empty:
                    st.info("💡 Desliza para ver las tarjetas individuales. ¡Toma una captura de pantalla para compartirlas en alta resolución!")
                    for _, row in df_cumple.iterrows():
                        ...
                        with st.expander(f"🎉 {row['Trabajador']} ({row['Fecha de cumpleaños']})"):
                            # Diseño calibrado para el nuevo fondo individual
                            st.markdown(f"""
                                <div style="position: relative; width: 100%; max-width: 500px; margin: auto; overflow: hidden; border-radius: 15px; box-shadow: 0px 4px 10px rgba(0,0,0,0.2);">
                                    ...
                                    <div style="position: absolute; top: 22%; left: 5%; right: 5%; text-align: center; font-family: sans-serif;">
                                        <b style="font-size: 1.8em; color: white; display: block; line-height: 1.1; text-shadow: 2px 2px 4px rgba(0,0,0,0.5);">{row['Trabajador']}</b>
                                        <span style="font-size: 1.2em; color: #FFD700; font-weight: bold; display: inline-block; margin-top: 8px; text-shadow: 1px 1px 2px rgba(0,0,0,0.8);">{row['Fecha de cumpleaños']}</span>
                                    </div>
                                    
                                    <div style="position: absolute; top: 48%; left: 8%; width: 130px; height: 130px; overflow: hidden; border-radius: 50%; border: 4px solid #FFD700; box-shadow: 3px 3px 10px rgba(0,0,0,0.4); background-color: white;">
                                        <img src="{foto_trabajador_url}" style="width: 100%; height: 100%; object-fit: cover;">
                                    </div>
                                </div>
                            """, unsafe_allow_html=True)
                            ...
                            email_add = str(row.get("Email", "")).strip()
                            
                            wa_url = f"https://wa.me/51{wa_num}?text=¡Feliz%20Cumpleaños,%20{row['Trabajador']}!%20🥳%20De%20parte%20de%20todo%20el%20equipo%20de%20la%20Universidad%20Roosevelt,%20esperamos%20que%20pases%20un%20día%20increíble." if wa_num and wa_num != "nan" else None
                            email_url = f"mailto:{email_add}?subject=¡Feliz%20Cumpleaños!%20🥳&body=¡Feliz%20Cumpleaños,%20{row['Trabajador']}!%20Esperamos%20que%20pases%20un%20día%20increíble.%20Te%20deseamos%20lo%20mejor." if email_add and email_add != "nan" else None

                            if wa_url or email_url:
                                st.markdown("<br>", unsafe_allow_html=True)
                                col_b1, col_b2 = st.columns(2)
                                with col_b1:
                                    if wa_url:
                                        st.markdown(f"""<a href="{wa_url}" target="_blank" style="display: block; width: 100%; padding: 10px; background-color: #25D366; color: white; text-align: center; font-weight: bold; border-radius: 8px; text-decoration: none;">📲 Enviar WhatsApp</a>""", unsafe_allow_html=True)
                                with col_b2:
                                    if email_url:
                                        st.markdown(f"""<a href="{email_url}" target="_blank" style="display: block; width: 100%; padding: 10px; background-color: #D44638; color: white; text-align: center; font-weight: bold; border-radius: 8px; text-decoration: none;">📧 Enviar Correo</a>""", unsafe_allow_html=True)
