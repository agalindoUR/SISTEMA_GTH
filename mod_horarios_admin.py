...
def mostrar(dfs, save_data=None):
    ...
    df_activos = df_contratos[df_contratos[col_estado[0]].astype(str).str.upper().str.contains('ACT')].copy() if col_estado else df_contratos.copy()
    ...
    col_nom = next((c for c in cols if c in ['NOMBRES', 'NOMBRE', 'NOMBRES Y APELLIDOS', 'APELLIDOS Y NOMBRES', 'TRABAJADOR']), None)
    ...
    def format_name(row):
        ...
        if n and a and (a in n or n in a): return n if len(n) > len(a) else a
    ...
    df_activos['DISPLAY'] = df_activos[col_dni].astype(str) + " - " + df_activos['NOMBRES_COMPLETOS'] + " (" + df_activos[col_cargo].astype(str).fillna('Sin cargo') + ")"
    ...
    if horario_bd is not None:
        st.success(f"📋 **Vigencia de Contrato Detectada:** Del `{f_inicio_contrato}` al `{f_fin_contrato}` | ✅ **Horario guardado previamente cargado.**")
    else:
        st.success(f"📋 **Vigencia de Contrato Detectada:** Del `{f_inicio_contrato}` al `{f_fin_contrato}` | 🆕 **Nuevo horario (valores por defecto).**")
    ...
    mapa_dias = {"Lunes":"LUN", "Martes":"MAR", "Miércoles":"MIE", "Jueves":"JUE", "Viernes":"VIE", "Sábado":"SAB", "Domingo":"DOM"}
    horarios_config = {}
    ...
    for dia in dias_semana:
        ...
        if horario_bd is not None:
            def_labora = (str(horario_bd.get(f"{prefijo}_LAB", "NO")).strip().upper() == "SI")
        ...
        if f"check_{dia}" not in st.session_state: st.session_state[f"check_{dia}"] = def_labora
        if f"tm_{dia}" not in st.session_state: st.session_state[f"tm_{dia}"] = def_tm
        if f"tt_{dia}" not in st.session_state: st.session_state[f"tt_{dia}"] = def_tt
        if f"e1_{dia}" not in st.session_state: st.session_state[f"e1_{dia}"] = val_e1
        if f"s1_{dia}" not in st.session_state: st.session_state[f"s1_{dia}"] = val_s1
        if f"e2_{dia}" not in st.session_state: st.session_state[f"e2_{dia}"] = val_e2
        if f"s2_{dia}" not in st.session_state: st.session_state[f"s2_{dia}"] = val_s2
            ...
            if asiste:
                ...
                with col_m:
                    ...
                    if turno_manana:
                        ...
                        with sub_c1: e1 = st.time_input("Entrada", key=f"e1_{dia}")
                        with sub_c2: s1 = st.time_input("Salida", key=f"s1_{dia}")
                ...
                with col_t:
                    ...
                    if turno_tarde:
                        ...
                        with sub_c3: e2 = st.time_input("Entrada", key=f"e2_{dia}")
                        with sub_c4: s2 = st.time_input("Salida", key=f"s2_{dia}")
